...
extension ObservableType {
    ...
    public func skip(_ duration: RxTimeInterval, scheduler: SchedulerType)
        -> Observable<E> {
        return SkipTime(source: asObservable(), duration: duration, scheduler: scheduler)
    }
}

// count version

fileprivate final class SkipCountSink<O: ObserverType>: Sink<O>, ObserverType {
...
fileprivate final class SkipCount<Element>: Producer<Element> {
...
fileprivate final class SkipTimeSink<ElementType, O: ObserverType>: Sink<O>, ObserverType where O.E == ElementType {
...
fileprivate final class SkipTime<Element>: Producer<Element> {
    let source: Observable<Element>
